    ...
    def step(self, action):
        ...
        if not player_valid_moves and not opponent_valid_moves:
            ...
            if white_score > black_score:
                ...
                reward = 1.0 # Reward relative to the player *whose turn it would have been*
            elif black_score > white_score:
                winner = game_consts.PLAYER_BLACK #
                reward = -1.0 # Assign reward based on the outcome for the player *whose turn it was*
                           # This needs careful consideration. Let's assign reward based on the player who *just* moved (or passed).
                           # If the current player is now WHITE (meaning BLACK just moved/passed), and BLACK wins, the reward for BLACK's last action is 1.0.
                           # So, reward should be based on the perspective of '-self.current_player'.
                # Let's redefine reward simply: +1 if the player who just acted wins, -1 if they lose, 0 for draw.
                if self.current_player == game_consts.PLAYER_WHITE: # Black just acted
                   reward = 1.0 if winner == game_consts.PLAYER_BLACK else (-1.0 if winner == game_consts.PLAYER_WHITE else 0.0)
                else: # White just acted
                   reward = 1.0 if winner == game_consts.PLAYER_WHITE else (-1.0 if winner == game_consts.PLAYER_BLACK else 0.0)
    ...
    for episode in tqdm(range(1, config.NUM_EPISODES + 1), desc="Training Episodes"): #
        ...
        while not done and episode_steps < config.MAX_STEPS_PER_EPISODE: #
            ...
            if not valid_moves:
                # Player must pass
                action = None
                action_index = action_to_index(action) # Get index for pass (should be 64)
            else:
                action, action_index = agent.select_action(processed_state, valid_moves) #

            accumulated_reward_for_step = 0 # Store reward for this step
            ...
            accumulated_reward_for_step = reward # Capture reward from env.step

            # --- MODIFICATION START ---
            if action is not None:
                # Only store experience and learn if a move was actually chosen by the agent
                # Store experience (state is from POV of player who *made* the move)
                agent.store_experience(processed_state, action_index, accumulated_reward_for_step, processed_next_state, done) #

                # Agent learns
                agent.learn() #
            # else:
            #    # If action was None (pass), do not store experience or learn
            #    pass # Explicitly do nothing here for clarity
            # --- MODIFICATION END ---

            # Update state and metrics (this happens whether a move was made or passed)
            processed_state = processed_next_state
            current_player = next_player # Update player for the next loop iteration
            episode_reward += accumulated_reward_for_step # Accumulate reward correctly
            episode_steps += 1
            total_steps += 1
